...
def save_results_to_file(train_metrics, test_metrics, feature_importance_data, 
    ...
    output_file = "../data/xgboost_results.txt"
    
    with open(output_file, 'w') as f:
        f.write("="*80 + "\n")
        f.write("XGBOOST MODEL RESULTS FOR USER ENGAGEMENT PREDICTION\n")
        f.write("="*80 + "\n\n")
        ...
        f.write(f"Confusion Matrix:\n")
        f.write(f"  True Negatives:  {cm[0,0]:,}\n")
        f.write(f"  False Positives: {cm[1,1]:,}\n")
        f.write(f"  False Negatives: {cm[1,0]:,}\n")
        f.write(f"  True Positives:  {cm[1,1]:,}\n\n")
        ...
        f.write("  - Built-in handling of missing values\n")
...
def main():
    ...
    print("="*60)
    print("XGBOOST MODEL FOR USER ENGAGEMENT PREDICTION")
    print("="*60)
    ...
    # Train model
    model, y_train_pred, y_test_pred, y_train_proba, y_test_proba = train_xgboost_model(
