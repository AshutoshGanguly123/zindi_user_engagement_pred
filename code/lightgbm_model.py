...
def train_lightgbm_model(X_train, X_test, y_train, y_test):
    ...
    print("Training initial LightGBM model...")
...
def save_results_to_file(train_metrics, test_metrics, feature_importance_data, 
    ...
    output_file = "../data/lightgbm_results.txt"
    
    with open(output_file, 'w') as f:
        f.write("="*80 + "\n")
        f.write("LIGHTGBM MODEL RESULTS FOR USER ENGAGEMENT PREDICTION\n")
        f.write("="*80 + "\n\n")
        ...
        f.write("-" * 40 + "\n")
        f.write("Model Type: LightGBM Classifier\n")
        f.write("Key Features:\n")
        f.write("  - Gradient boosting with leaf-wise tree growth\n")
        f.write("  - Faster training and lower memory usage than XGBoost\n")
...
def main():
    ...
    print("="*60)
    print("LIGHTGBM MODEL FOR USER ENGAGEMENT PREDICTION")
    print("="*60)
    ...
    # Train model
    model, y_train_pred, y_test_pred, y_train_proba, y_test_proba = train_lightgbm_model(
    ...
    # Save model
    import joblib
    joblib.dump(model, "../data/lightgbm_model.pkl")
